...
var reactCompilerRuntime = require("react-compiler-runtime"), React = require("react"), useEditor = require("../_chunks-cjs/use-editor.cjs"), react = require("@xstate/react"), isEqual = require("lodash/isEqual.js"), xstate = require("xstate"), selector_getFocusSpan = require("../_chunks-cjs/selector.get-focus-span.cjs"), util_sliceBlocks = require("../_chunks-cjs/util.slice-blocks.cjs"), util_childSelectionPointToBlockOffset = require("../_chunks-cjs/util.child-selection-point-to-block-offset.cjs"), selector_getTextBefore = require("../_chunks-cjs/selector.get-text-before.cjs"), behaviors_index = require("../behaviors/index.cjs"), useEffectEvent = require("use-effect-event"), jsxRuntime = require("react/jsx-runtime"), util_mergeTextBlocks = require("../_chunks-cjs/util.merge-text-blocks.cjs");
  ...
  return behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      if (prefixOffsets.focus.offset - prefixOffsets.anchor.offset > 1) {
        ...
        }), inlineObjectBeforePrefixFocus = selector_getFocusSpan.getPreviousInlineObject({
          ...snapshot,
          context: {
            ...snapshot.context,
      ...
      if (suffixOffsets.focus.offset - suffixOffsets.anchor.offset > 1) {
        const previousInlineObject = selector_getFocusSpan.getPreviousInlineObject(snapshot), previousInlineObjectOffset = previousInlineObject ? util_childSelectionPointToBlockOffset.childSelectionPointToBlockOffset({
          context: {
            schema: snapshot.context.schema,
            value: snapshot.context.value
          },
  ...
  const automaticBlockquoteOnSpace = behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const previousInlineObject = selector_getFocusSpan.getPreviousInlineObject(snapshot), blockOffset = util_sliceBlocks.spanSelectionPointToBlockOffset({
        context: snapshot.context,
        selectionPoint: {
          path: [{
    ...
    guard: ({
      ...
      if (hrCharacter === void 0)
        return !1;
      const hrObject = config.horizontalRuleObject?.(snapshot.context), focusBlock = selector_getFocusSpan.getFocusTextBlock(snapshot), selectionCollapsed = selector_getFocusSpan.isSelectionCollapsed(snapshot);
      if (!hrObject || !focusBlock || !selectionCollapsed)
        return !1;
      const previousInlineObject = selector_getFocusSpan.getPreviousInlineObject(snapshot), textBefore = selector_getTextBefore.getBlockTextBefore(snapshot), hrBlockOffsets = {
        anchor: {
    ...
    guard: ({
      ...
      const text = event.originEvent.dataTransfer.getData("text/plain"), hrRegExp = /^(---)$|(___)$|(\*\*\*)$/, hrCharacters = text.match(hrRegExp)?.[0], hrObject = config.horizontalRuleObject?.(snapshot.context), focusBlock = selector_getFocusSpan.getFocusBlock(snapshot);
    ...
    guard: ({
      ...
      if (!blockOffset)
        return !1;
      const previousInlineObject = selector_getFocusSpan.getPreviousInlineObject(snapshot), blockText = util_sliceBlocks.getTextBlockText(focusTextBlock.node), markdownHeadingSearch = /^#+/.exec(blockText), level = markdownHeadingSearch ? markdownHeadingSearch[0].length : void 0, caretAtTheEndOfHeading = blockOffset.offset === level;
...
const oneLineBehaviors = [
  ...
  behaviors_index.defineBehavior({
    ...
    guard: ({
      snapshot,
      event
    }) => !(!selector_getFocusSpan.getFocusTextBlock(snapshot) || !util_mergeTextBlocks.isTextBlock(snapshot.context, event.block)),
    actions: [({
      event
    }) => [behaviors_index.execute({
      type: "insert.block",
